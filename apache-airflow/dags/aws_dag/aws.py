...
load_dotenv('apache-airflow/.env')
...
class AWS_Airflow(ABC):
    ...
    def iniciar(self) -> None:
        if self._valida_bucket():
            self._criar_bucket('rri-s3-bucket-aula-airflow')
            print(f'bucket riado com suceso: rri-s3-bucket-aula-airflow')
        else:
    ...
    def enviar_arquivo(self) -> None:
        ...
        self.s3_resource.Bucket('rri-s3-bucket-aula-airflow').upload_file(
